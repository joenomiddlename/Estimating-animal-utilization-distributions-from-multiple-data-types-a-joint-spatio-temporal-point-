...
if(test_code == T)
{
  simul <- go(animal,one_boat, 500, sigma=sigma_Brownian, sigma_observer = sigma_Brownian_observer_lowbias, n_keep = 10, Brillinger=T, r=10, dist_fun = distance_function, observers_UD = observer_UD_lowbias)  
  ...
  detection_prob <- rep(NA, 500)
  detection_prob_oneboat <- rep(NA, 500)
  for(i in 1:500) 
  {
    print(i)
    simul <- go(animal, one_boat, 500, sigma=sigma_Brownian, sigma_observer = sigma_Brownian_observer_lowbias, n_keep = 10, Brillinger=T, r=10, dist_fun = distance_function, observers_UD = observer_UD_lowbias)  
    simul2 <- go(animal, twenty_boats, 500, sigma=sigma_Brownian, sigma_observer = sigma_Brownian_observer_lowbias, n_keep = 10, Brillinger=T, r=10, dist_fun = distance_function, observers_UD = observer_UD_lowbias)  
    # how far away was the animal from the closest observer?
    if(!is.null(simul2$detected_point))
    {
      last_ind <- dim(simul2$track_observers)[2]
      last_obs_positions <- simul2$track_observers[,last_ind,]
      if(last_ind==1)
      {
        last_animal_position <- t(simul2$track)
      }
      if(last_ind!=1)
      {
        last_animal_position <- t(simul2$track[last_ind,])
      }
      detection_prob[i] <- 1 - (prod(1 - distance_function(as.matrix(dist(rbind(last_obs_positions,last_animal_position)))[21,-c(21)],10)))
    }
    if(!is.null(simul$detected_point))
    {
      # repeat for one boat
      last_ind <- dim(simul$track_observers)[2]
      last_obs_positions <- t(simul$track_observers[,last_ind,])
      if(last_ind==1)
      {
        last_animal_position <- t(simul$track)
      }
      if(last_ind!=1)
      {
        last_animal_position <- t(simul$track[last_ind,])
      }
      detection_prob_oneboat[i] <- 1 - (prod(1 - distance_function(as.matrix(dist(rbind(last_obs_positions,last_animal_position)))[2,-c(2)],10)))
    }
  }
  hist(detection_prob)
  hist(detection_prob_oneboat)
  summary(detection_prob)
  summary(detection_prob_oneboat)
...
simulator <- function(M, sp, sp2, n, sigma, sigma_observer, n_keep, Brillinger=T, r=1e9, dist_fun=function(x){1}, observers_UD)
{
  ...
  for(i in 1:M)
  {
    ...
    observer_tracks <- abind(observer_tracks, result$track_observers, along = 2)
...
if(test_code==T)
{
  ...
  approx_tracks <- over(results$observer_tracks, effort)
  # compute the approx distance between each observer track point and the pixels
  # Need to loop to avoid memory crash
  effort_vals <- rep(0, dim(dist_mat_pixels)[1])
  for(ind in 1:(dim(dist_mat_pixels)[1]))
  {
    effort_vals[ind] <- sum(distance_function(d=dist_mat_pixels[ind,approx_tracks],r=10))
  }
  ...
  for(ind in 1:(dim(dist_mat_pixels)[1]))
  {
    effort_vals2[ind] <- sum(distance_function(d=dist_mat_pixels[ind,approx_tracks2],r=10))
  }
  
  effort2$effort <- log(effort_vals2+1e-5)
...
twenty_boats <- observers(x=rep(75,20),y=rep(50,20), n=20, static=rep(F, 20))
observers_sim <- list(one_boat, twenty_static, twenty_boats, one_boat_twenty_static)
# set-up different observer biases
sigma_Brownian_observer_sim <- c(sqrt(2), sqrt(8))
...
)
{
  ...
  for(ind in 1:(dim(dist_mat_pixels)[1]))
  {
    effort_vals[ind] <- sum(distance_function_assumed(d=dist_mat_pixels[ind,approx_tracks],r=detect_range_val))
  }
...
Sim_Results <- array(0, dim = c(length(observers_sim), length(sigma_Brownian_observer_sim), length(n_trips_sim), length(detect_surface_sim), length(detect_range_sim), n_rep_sim, 9),
                     dimnames = list(Observers=c('one_boat', 'twenty_static', 'twenty_boats', 'one_boat_twenty_static'),
...
for(observers in observers_sim)
{
  ...
  for(sigma_B in sigma_Brownian_observer_sim)
  {
    ...
    saveRDS(Sim_Results, 'Encounter_Sim_Results_New.rds')
  }
  count1 <- count1 + 1
}
